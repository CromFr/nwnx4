...
int NWNXGetInt(char* sPlugin, char* sFunction, char* sParam1, int nParam2)
{
	...
	auto cpluginIt = plugins.find(pluginClass);
	if (cpluginIt != plugins.end())
	{
		return cpluginIt->second->GetInt(sFunction, sParam1, nParam2);
	}
...
void NWNXSetFloat(char* sPlugin, char* sFunction, char* sParam1, int nParam2, float fValue)
{
	...
	if (pluginIt != plugins.end())
	{
		pluginIt->second->SetFloat(sFunction, sParam1, nParam2, nValue);
		return;
	}
...
char* NWNXGetString(char* sPlugin, char* sFunction, char* sParam1, int nParam2)
{
	...
	if (cpluginIt != plugins.end())
	{
		return cpluginIt->second->GetString(sFunction, sParam1, nParam2);
	}
...
void NWNXSetString(char* sPlugin, char* sFunction, char* sParam1, int nParam2, char* sValue)
{
	...
	if (pluginIt != plugins.end())
	{
		pluginIt->second->SetString(sFunction, sParam1, nParam2, nValue);
		return;
	}
...
void loadPlugins()
{
	...
	for(auto& pluginPath : pluginList){
		...
		if (cpluginABIVersion != nullptr) {
			try {
				CPluginInitInfo initInfo{
					pluginPathStr.c_str(),
					nwnInstallHome->c_str(),
					nwnxHome->c_str()
				};
				...
				if(cplugin.GetIsLoaded()){
					auto className = cplugin->GetClassName();
					if(cplugins.contains(className)){
						logger->Err("* Error: Two or more plugins are registered with the class %s", className.c_str());
					}
					else{
						cplugins[className] = cplugin;
						logger->Debug("* Loaded C plugin %s: Successfully registered", pluginName.c_str());
					}
				}
				else{
					logger->Err("* Failed to initialize plugin %s", pluginName.c_str());
				}
			} catch (const std::exception& exception) {
				logger->Err("* Exception thrown while loading plugin %s: %s", pluginName.c_str(), exception.what());
			}
